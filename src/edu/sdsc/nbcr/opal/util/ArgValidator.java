...
import edu.sdsc.nbcr.opal.types.ParamType;
import edu.sdsc.nbcr.opal.types.ParamsType;
import edu.sdsc.nbcr.opal.types.ImplicitParamsType;
import edu.sdsc.nbcr.opal.types.GroupsType;
import edu.sdsc.nbcr.opal.types.FlagsType;
import edu.sdsc.nbcr.opal.types.IOType;
...
public class ArgValidator {
	...
	private boolean validateParamType(String current,
		...
		if (type.getValue().equals(ParamType._INT)) {
			try {
				...
				logger.error(e);
				throw new OpalFaultResponse("Parameter " + id + " expects integer value");
			}
			...
			try {
				...
				logger.error(e);
				throw new OpalFaultResponse("Parameter " + id + " expects boolean value");
			}
			...
			try {
				...
				logger.error(e);
				throw new OpalFaultResponse("Parameter " + id + " expects float value");
			}
			...
			if (ioType.getValue().equals(IOType._INPUT) ||
				...
				if (test.exists()) {
					...
					logger.error("File parameter: " + filePath + " for tag " +
							id + " doesn't exist");
					throw new OpalFaultResponse("File parameter: " + filePath + " for tag " +
							id + " doesn't exist");
				}
			} else {
			...
			try {
				...
				logger.error(e);
				throw new OpalFaultResponse("Parameter " + id + " expects a valid URL");
			}
		}
	 ...
	 * @throws OpalFaultResponse if there was an error during argument validation
	 */
	public boolean validateArgList(String workingDir,
		...
		if (!args.equals("")) {
			...
			while (index < argList.length) {
					...
					if (untaggedIndex < untaggedParams.length) {
						...
						if (groupMap.containsKey(id.toString())) {
							...
							if (group == null) {
								...
								throw new OpalFaultResponse("Can't find group in hash table: " + groupName);
							}
						}
				...
				throw new OpalFaultResponse("No match found for argument: " + current);
			}
		}
		...
		if (taggedParams != null) {
			for (int i = 0; i < taggedParams.length; i++) {
				...
				if ((required) && (!present.contains(id))) {
					...
					throw new OpalFaultResponse("Required parameter " + id + " not found");
				}
			}
		}
		if (untaggedParams != null) {
			for (int i = 0; i < untaggedParams.length; i++) {
				...
				if ((required) && (!present.contains(id))) {
					...
					throw new OpalFaultResponse("Required untagged parameter not found");
				}
			}
		}
						...
						if (present.contains(new Id(elemIDs[j]))) {
							if (exclusive) {
								logger.error("Found multiple parameters inside exclusive group: " +
										groups[i].getName());
								throw new OpalFaultResponse("Found multiple parameters inside exclusive group: " +
										groups[i].getName());
							} else
								exclusive = true;
						}
	 ...
	 * @throws OpalFaultResponse if there in an error during implicit param validation
	 */
	public boolean validateImplicitParams(String workingDir)
		...
		if (!dir.exists()) {
			logger.error("Working directory " + workingDir +
					" does not exist");
			throw new OpalFaultResponse("Working directory " + workingDir +
					" does not exist");
		}
		...
		if (!dir.isDirectory()) {
			...
			throw new OpalFaultResponse("Specified working directory " + workingDir +
					" is not a valid directory");
		}
		...
		for (int i = 0; i < params.length; i++) {
			if (params[i].getRequired()) {
				if (present.containsKey(params[i].getId())) {
					...
					throw new OpalFaultResponse("Required implicit parameter " + params[i].getId() +
							" missing from working directory");
				}
			}
			...
			if (num < min) {
				...
				throw new OpalFaultResponse("Number of implicit parameters " + params[i].getId() +
						" less than minimum (" + min + ")");
			} else {
	...
	public static void main(String args[])
		...
		//AppConfigType appConfig = (AppConfigType) TypeDeserializer.getValue(configFile,
		//		new AppConfigType());
		InputStream in = new FileInputStream(configFile);
		XMLInputFactory factory = XMLInputFactory.newInstance();
		XMLStreamReader appconfig_parser = factory.createXMLStreamReader(in);
		AppConfigType appConfig = AppConfigType.Factory.parse(appconfig_parser);
