...
public class Deploy {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            //					  new AppConfigType());
            InputStream in = new FileInputStream(appConfig);
            XMLInputFactory factory = XMLInputFactory.newInstance();
            XMLStreamReader parser = factory.createXMLStreamReader(in);
            config = AppConfigType.Factory.parse(parser);
            //System.out.println(config.getMetadata().getAppName());
            //System.out.println(config.getMetadata().getInfo()[0]);
            //System.out.println(config.getMetadata().getUsage());
            //System.out.println(config.getMetadata().getTypes().getTaggedParams().getParam()[0].getId());
            //System.out.println(config.getMetadata().getTypes().getTaggedParams().getParam()[0].get_default());
