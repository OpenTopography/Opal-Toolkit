...
public class AppMetadataParser {
    ...
    static public AppMetadata parseAppMetadata(String serviceURL) {
        ...
        try {
            AppServiceStub appServicePort = new AppServiceStub(serviceURL);
            GetAppConfigInput appconf = new GetAppConfigInput();
            AppConfigInputType appconf_in = new AppConfigInputType();
            appconf.setGetAppConfigInput(appconf_in);
            GetAppConfigOutput appconf_out = appServicePort.getAppConfig(appconf);
            AppConfigType act = appconf_out.getGetAppConfigOutput();
    ...
    static private Group parseGroup(GroupsType groupType, AppMetadata app) {
        ...
        if (groupType.getExclusive()) group.setExclusive(groupType.getExclusive());
        if (groupType.getRequired()) {
