...
public class LaunchJobAction extends MappingDispatchAction {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        try {
            AppServiceStub appServicePort = new AppServiceStub(app.getURL());
            LaunchJobInput lj_in = new LaunchJobInput();
            lj_in.setLaunchJobInput(in);
            // TODO: fix this to use attachments, if need be
            LaunchJobOutput lj_out = appServicePort.launchJob(lj_in);
            subOut = lj_out.getLaunchJobOutput();
            if (subOut == null) {
     ...
     *
     * @param app provide app metadata
        ...
        try {
            ...
            for (int i = 0; i < al.size(); i++) { // get files from the bean
                ...
                if (osFile != null) {
                    ...
                    file.setAttachment((OMElement) dh);
                } else {
                    DataHandler dh = new DataHandler(new ByteArrayDataSource(ff.getFileData()));
                    file.setContents(dh);
                }
            ...
            if (app.getNumArgFileSubmitted() > 0) { //we have some files in the argParam array...
                ...
                for (int i = 0; i < numFile; i++) {
                    ...
                    if (ff != null) {
                        ...
                        File osFile = getStoredFile(ff);
                        if (osFile != null) {
                            javax.activation.DataHandler dh = new javax.activation.DataHandler(new FileDataSource(osFile));
                            files[i].setAttachment((OMElement) dh);
                        } else { // data is in memory already - reuse it
                            ByteArrayDataSource rawData = new ByteArrayDataSource(ff.getFileData());
                            DataHandler dh_bin = new DataHandler(rawData);
                            files[i].setContents(dh_bin);
                        }
                    } else {
