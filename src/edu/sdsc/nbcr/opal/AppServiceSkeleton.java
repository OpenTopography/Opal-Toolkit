...
import edu.sdsc.nbcr.opal.util.InputStreamDataSource;

/**
 *  AppServiceSkeleton java skeleton for the axisService
 *
 * @author Choonhan Youn
 */
/** AppServiceSkeleton java skeleton for the axisService */
public class AppServiceSkeleton {

  // get an instance of the log4j Logger
  private static Logger logger =
          Logger.getLogger(AppServiceSkeleton.class.getName());
  ...
  static {
    ...
    try {
      props.load(AppServiceSkeleton.class.getClassLoader().
   ...
   * Default constructor
   *
   * @throws OpalFaultResponse if there is an error during initialization
   */
  public AppServiceSkeleton()
          throws OpalFaultResponse {
    logger.debug("called");

    if (tomcatURL == null) {
      logger.fatal("Can't find property: tomcatURL");
      throw new OpalFaultResponse("Can't find property: tomcatURL");
    }
    if (catalinaHome == null) {
      logger.fatal("Can't find property: catalina.home");
      throw new OpalFaultResponse("Can't find property: catalina.home");
    }
  }
   ...
   * @throws OpalFaultResponse if there is an error in output retrieval
   */
  public edu.sdsc.nbcr.opal.types.GetOutputsOutput getOutputs(
      edu.sdsc.nbcr.opal.types.GetOutputsInput in) throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    String jobId = in.getGetOutputsInput();
    ...
    try {
      ...
      throw new OpalFaultResponse(se.getMessage());
    }
    ...
    if (!test.exists()) {
      ...
      throw new OpalFaultResponse("Job outputs have been cleaned up");
    }

    GetOutputsOutput job_output = new GetOutputsOutput();
    job_output.setGetOutputsOutput(outputs);
   ...
   * and output metadata
   * @throws OpalFaultResponse if there is an error during job submission
   */
  public edu.sdsc.nbcr.opal.types.LaunchJobBlockingOutput launchJobBlocking(
      edu.sdsc.nbcr.opal.types.LaunchJobBlockingInput in)
      throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    logger.info("called");
    ...
    String jobID = launchApp(in.getLaunchJobBlockingInput(), true);

    // create output object
    BlockingOutputType output = new BlockingOutputType();
    QueryStatusInput status_in = new QueryStatusInput();
    status_in.setQueryStatusInput(jobID);
    QueryStatusOutput status_out = queryStatus(status_in);
    StatusOutputType status = status_out.getQueryStatusOutput();

    output.setStatus(status);
    GetOutputsInput output_in = new GetOutputsInput();
    output_in.setGetOutputsInput(jobID);
    GetOutputsOutput jobOut = getOutputs(output_in);
    output.setJobOut(jobOut.getGetOutputsOutput());

    LaunchJobBlockingOutput b_output = new LaunchJobBlockingOutput();
    b_output.setLaunchJobBlockingOutput(output);
    long t1 = System.currentTimeMillis();
   ...
   * @throws OpalFaultResponse if there is an error during retrieval of application configuration
   */
  public edu.sdsc.nbcr.opal.types.GetAppConfigOutput getAppConfig(
      edu.sdsc.nbcr.opal.types.GetAppConfigInput in) throws OpalFaultResponse {
    logger.info("called");
    // make sure that the config has been retrieved
    retrieveAppConfig();
    // return the config
    GetAppConfigOutput appconfig_output = new GetAppConfigOutput();
    appconfig_output.setGetAppConfigOutput(config);
    return appconfig_output;
  }
   ...
   * @throws OpalFaultResponse if there is an error during the status query
   */
  public edu.sdsc.nbcr.opal.types.QueryStatusOutput queryStatus(
      edu.sdsc.nbcr.opal.types.QueryStatusInput in) throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    String jobId = in.getQueryStatusInput();
    logger.info("Query status for job: " + jobId);
    ...
    try {
      status = HibernateUtil.getStatus(jobId);
    } catch (StateManagerException se) {
      logger.error(se.getMessage());
      throw new OpalFaultResponse(se.getMessage());
    }

    QueryStatusOutput status_output = new QueryStatusOutput();
    status_output.setQueryStatusOutput(status);
    long t1 = System.currentTimeMillis();
   ...
   * @throws OpalFaultResponse if there is an error during job destruction
   */
  public edu.sdsc.nbcr.opal.types.DestroyOutput destroy(
      edu.sdsc.nbcr.opal.types.DestroyInput in) throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    String jobId = in.getDestroyInput();
    logger.info("called for job: " + jobId);
    ...
    if (jobTable.containsKey(in)) {
      ...
      try {
        ...
        logger.error(msg);
        throw new OpalFaultResponse(msg);
      }
    } else {
      ...
      status_in.setQueryStatusInput(jobId);
      QueryStatusOutput status_out = queryStatus(status_in);
      status = status_out.getQueryStatusOutput();
    }
    ...
    if (status.getBaseURL() == null) {
      try {
        ...
        throw new OpalFaultResponse(message);
      }
    }

    DestroyOutput d_output = new DestroyOutput();
    d_output.setDestroyOutput(status);
    long t1 = System.currentTimeMillis();
   ...
   * @throws OpalFaultResponse if there is an error during job submission
   */
  public edu.sdsc.nbcr.opal.types.LaunchJobOutput launchJob(
      edu.sdsc.nbcr.opal.types.LaunchJobInput in) throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    logger.debug("called");
    ...
    String jobID = launchApp(in.getLaunchJobInput(), false);
    ...
    LaunchJobOutput lj_output = new LaunchJobOutput();
    lj_output.setLaunchJobOutput(output);
    long t1 = System.currentTimeMillis();
    logger.debug("Server execution time: " + (t1-t0) + " ms");
    return lj_output;
  }
   ...
   * @throws OpalFaultResponse if there is an error during the status query
   */
  public edu.sdsc.nbcr.opal.types.GetJobStatisticsOutput getJobStatistics(
      edu.sdsc.nbcr.opal.types.GetJobStatisticsInput in)
      throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    String jobId = in.getGetJobStatisticsInput();
    logger.info("called for job: " + jobId);
    ...
    try {
      stats = HibernateUtil.getStatistics(jobId);
    } catch (StateManagerException se) {
      logger.error(se.getMessage());
      throw new OpalFaultResponse(se.getMessage());
    }

    GetJobStatisticsOutput js_output = new GetJobStatisticsOutput();
    js_output.setGetJobStatisticsOutput(stats);
    long t1 = System.currentTimeMillis();
    logger.debug("Query execution time: " + (t1-t0) + " ms");
    return js_output;
  }
   ...
   * @throws OpalFaultResponse if there is an error during retrieval of system information
   */
  public edu.sdsc.nbcr.opal.types.GetSystemInfoOutput getSystemInfo(
      edu.sdsc.nbcr.opal.types.GetSystemInfoInput in) throws OpalFaultResponse {
    logger.info("called");
    ...
    GetSystemInfoOutput sys_info_out = new GetSystemInfoOutput();
    sys_info_out.setGetSystemInfoOutput(result);
   ...
   * @throws OpalFaultResponse if there is an error during retrieval of application metadata
   */
  public edu.sdsc.nbcr.opal.types.GetAppMetadataOutput getAppMetadata(
      edu.sdsc.nbcr.opal.types.GetAppMetadataInput in) throws OpalFaultResponse {
    logger.debug("called");

    // make sure that the config has been retrieved
    retrieveAppConfig();

    GetAppMetadataOutput am_output = new GetAppMetadataOutput();
    am_output.setGetAppMetadataOutput(config.getMetadata());
    // return the metadata
    return am_output;
  }
   ...
   * @throws OpalFaultResponse if there is an error in output generation
   */
  public edu.sdsc.nbcr.opal.types.GetOutputAsBase64ByNameOutput getOutputAsBase64ByName(
      edu.sdsc.nbcr.opal.types.GetOutputAsBase64ByNameInput in)
      throws OpalFaultResponse {
    long t0 = System.currentTimeMillis();
    String jobId = in.getGetOutputAsBase64ByNameInput().getJobID();
    String filename = in.getGetOutputAsBase64ByNameInput().getFileName();
    logger.info("called for job: " + jobId + " with file name: " + filename);

    //byte[] data = null;
    DataHandler dataHandler = null;
    String outputDirName = outputPrefix + File.separator + jobId + File.separator;
    String outputURL = outputDirName + filename;
    File f = new File(outputURL);
    if (f.exists()) {
      try {
        //data = new byte[(int) f.length()];
        FileInputStream fIn = new FileInputStream(f);
        dataHandler = new DataHandler(new InputStreamDataSource(fIn));
        //fIn.read(data);
        //fIn.close();
      } catch (Exception e) {
        logger.error("Error while trying to read output: " + e.getMessage());
        throw new OpalFaultResponse("Error while trying to read output: " + e.getMessage());
      }

    } else {
      logger.error("File " + filename + " doesn't exist on server");
      throw new OpalFaultResponse("File " + filename + " doesn't exist on server");
    }

    GetOutputAsBase64ByNameOutput base64_output = new GetOutputAsBase64ByNameOutput();
    base64_output.setGetOutputAsBase64ByNameOutput(dataHandler);
    long t1 = System.currentTimeMillis();
    logger.debug("Output retrieval time: " + (t1-t0) + " ms");
  ...
  private String launchApp(JobInputType in,
    ...
    if (!outputDir.mkdir()) {
      ...
      throw new OpalFaultResponse("Can't create new directory to run application in" + outputDir);
    }
    ...
    try {
      ...
      throw new OpalFaultResponse(jme.getMessage());
    }
    ...
    // special remote slurm job manager
    if (jobManagerFQCNLocal.equals("edu.sdsc.nbcr.opal.manager.SlurmJobManager")) {
      if (!blocking) {
        logger.info("Remote Slurm job ............");
        // launch thread to monitor status
        new Thread() {
          public void run() {
            try {
              manageSlurmJob(jobManager,
                      jobID,
                      outputDirName,
                      baseURL,
                      handle);
  ...
  private synchronized void manageSlurmJob(OpalJobManager jobManager,
                                           String jobID,
                                           String workingDir,
                                           URI baseURL,
                                           String handle)
    ...
    try {
      ...
      throw new OpalFaultResponse(e.getMessage());
    }
  ...
  private void writeInputFile(String outputDirName,
    ...
    try {
      ...
      if (inputFile.getContents() != null) {
        //it is a 'normal' file
        DataHandler input_dh = inputFile.getContents();
        FileOutputStream fileOutputStream = new FileOutputStream(f);
        input_dh.writeTo(fileOutputStream);
        ...
        // it is an attachment
        DataHandler dh = (DataHandler) inputFile.getAttachment();
      ...
      logger.error("IOException while trying to write input file: " +
              ioe.getMessage());
      throw new OpalFaultResponse("IOException while trying to write input file: " +
              ioe.getMessage());
    }
  }
  ...
  /**
   * @return returns "true" if IP processing is off
  ...
  private void retrieveAppConfig()
    ...
    Map<String, Object> props = mc.getProperties();
    for (String s : props.keySet())
      System.out.println("Property: "+s+":"+props.get(s));
    ...
    String configFileName = (String) service.getParameterValue("appConfig");
    if (configFileName == null) {
      logger.error("Required parameter appConfig not found in WSDD");
      throw new OpalFaultResponse("Required parameter appConfig not found in WSDD");
    }
    ...
    if (reconfigure) {
      ...
      try {
        ...
        config = AppConfigType.Factory.parse(parser);
      } catch (Exception e) {
        logger.error(e);
