...
public class MetaServiceJobManager implements OpalJobManager {
    ...
    private AppServiceStub appServicePort;
    ...
    public String launchJob(String argList,
        ...
        try {
            ...
            appServicePort = new AppServiceStub(remote_url);

            LaunchJobInput lj_in = new LaunchJobInput();
            lj_in.setLaunchJobInput(in);
            LaunchJobOutput lj_subOut = appServicePort.launchJob(lj_in);
            ...
            qs_in.setQueryStatusInput(remoteJobID);
            QueryStatusOutput qs_status = appServicePort.queryStatus(qs_in);
            StatusOutputType remoteStatus = qs_status.getQueryStatusOutput();
    ...
    public StatusOutputType waitForActivation()
        ...
        try {
            ...
            while (code == GramJob.STATUS_PENDING) {
				qs_in.setQueryStatusInput(remoteJobID);
				qs_status = appServicePort.queryStatus(qs_in);
				status = qs_status.getQueryStatusOutput();
                code = status.getCode();
                Thread.sleep(3000);
            }
        } catch (RemoteException e) {
    ...
    public StatusOutputType waitForCompletion()
        ...
        try {
			...
			out_in.setGetOutputsInput(remoteJobID);
			GetOutputsOutput out_out = appServicePort.getOutputs(out_in);
    ...
    public StatusOutputType destroyJob()
        ...
        try {
			...
			d_in.setDestroyInput(remoteJobID);
			DestroyOutput d_status = appServicePort.destroy(d_in);
